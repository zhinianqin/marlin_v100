...
`marlin_v100` 是一个独立的最小 SM70 / V100 Marlin 开发项目，用于在较小工作区内开发、整理和验证以下能力：
...
这里的主要目标是提升 Marlin 重构效率，而不是复制完整的 vLLM 主树能力。
...
- `tests/`
  放置本地独立测试，不默认回写主树。
- `benchmarks/`
  放置本地 benchmark、shape 枚举和分析脚本。benchmark artifact 不默认回写主树。
- `docs/`
  放置当前策略文档与历史实验记录。日期型实验文档保留历史语义，不一定代表当前实现。
- `upstream_map.yaml`
  放置主树回写映射，是回写时的唯一依据。
- `cmake/`
...
- 文档中的日期型 benchmark / probe / bugfix 记录是历史 artifact；当前实现策略优先看最新策略文档、源码和测试 helper。
...
- 文档或窄测试 helper 改动：
  通常只需要 `git diff --check` 和相关静态检查。
- CUDA/C++ 源码改动：
  至少运行 `./build.sh` 和导入 / op 注册检查。
- 当前 SM70 机器：
  可用于构建、导入、op 注册和必要的 SM70 runtime smoke。
- pytest / exhaustive env sweep：
  按任务需要运行；它们不是每次改动的默认硬门槛，full sweep 可能非常慢。
...
- 不要把本地文档、测试、benchmark、辅助封装当作上游源码一并回写。
- 不要把历史日期型文档里的旧策略当成当前事实；当前事实以源码、测试 helper 和最新策略文档为准。
- 不要在没有明确要求时主动提交代码。
