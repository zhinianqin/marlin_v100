...
if not SUPPORT_SM75 or any(arch != 75 for arch in ARCHS):
    raise RuntimeError("This generator only supports a single SM75 target.")
...
def generate_new_kernels():
    ...
    for (a_type, b_type, c_type), config_list in result_dict.items():
        ...
        filename = f"sm75_kernel_{a_type[1:]}_{b_type[1:]}_{c_type[1:]}.cu".lower()
    ...
    if kernel_selector_str != FILE_HEAD_COMMENT:
        kernel_selector_str += (
            ...
            '  { TORCH_CHECK(false, "marlin moe bf16 kernels are not built for SM75."); }\n'
            "else if (a_type == vllm::kFE4M3fn || b_type == vllm::kFE4M3fn || "
            "b_type == vllm::kFE2M1f || s_type == vllm::kFE4M3fn || "
            "s_type == vllm::kFE8M0fnu)\n"
            '  { TORCH_CHECK(false, "marlin moe fp8 kernels are not built for SM75."); }'
        )
