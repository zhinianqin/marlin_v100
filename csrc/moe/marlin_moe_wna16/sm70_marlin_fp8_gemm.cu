...
constexpr char const* kSm70MarlinMoeFp8CtaEnv =
    "SM70_MARLIN_MOE_FP8_CTA";
constexpr char const* kSm70MarlinMoeFp8SplitKEnv =
    "SM70_MARLIN_MOE_FP8_SPLIT_K";
...
class Sm70MoeFp8IteratorB {
 public:
  ...
  static_assert(Shape::kK == kCtaK,
                "SM70 Marlin MoE FP8 IteratorB expects CTA_K=32.");
...
using Sm70MoeFp8GemmTraits =
    Sm70MarlinMoeGemmTraits<Sm70MoeFp8GemmSpec, CtaM, CtaN, Warps,
                            GroupSize>;
...
struct Sm70MoeFp8Launcher {
  ...
  torch::Tensor operator()() const {
    using Traits = Sm70MoeFp8GemmTraits<CtaM, CtaN, Warps, GroupSize>;
    return launch_sm70_marlin_moe_gemm<Traits>(
        ...
        kSm70MarlinMoeFp8SplitKEnv, moe_block_size, top_k, mul_topk_weights,
...
torch::Tensor sm70_marlin_fp8_gemm(
  ...
  Sm70CtaGeometry const geometry =
      resolve_sm70_marlin_moe_cta_geometry(kSm70MarlinMoeFp8CtaEnv, size_n);
  check_sm70_marlin_moe_cta_geometry(kSm70MarlinMoeFp8CtaEnv, geometry);
  check_sm70_marlin_moe_cta_n_alignment(kSm70MarlinMoeFp8CtaEnv, geometry,
                                        size_n);
  TORCH_CHECK(size_k % kCtaK == 0,
              "SM70 Marlin MoE FP8 requires K divisible by 32. Got K=",
              size_k, ".");

  int const split_k = parse_sm70_split_k(kSm70MarlinMoeFp8SplitKEnv);
  ...
  Sm70MoeFp8Launcher const launcher{
  ...
  return dispatch_sm70_marlin_moe_fp8_geometry(launcher, geometry, group_size);
}
