...
torch::Tensor sm70_marlin_u4_gemm(
  ...
  if (params.use_metadata_vector_words) {
    Sm70MoeU4Launcher<HasBias, true> const launcher{
  ...
  Sm70MoeU4Launcher<HasBias, false> const launcher{
      a, c, b_q_weight, b_scales, b_zeros, b_bias, empty_float, sorted_token_ids,
      expert_ids, num_tokens_past_padded, topk_weights, moe_block_size, top_k,
      mul_topk_weights, size_m, size_n, size_k, params.requested_split_k};
  return dispatch_sm70_marlin_moe_geometry(launcher, geometry, params.packed_macro_n, group_size,
...
template torch::Tensor marlin_moe_wna16::sm70_marlin_u4_gemm<false>(
    torch::Tensor& a, torch::Tensor& c, torch::Tensor& b_q_weight,
    torch::Tensor& b_scales, torch::Tensor& b_zeros, torch::Tensor& b_bias,
    torch::Tensor& sorted_token_ids, torch::Tensor& expert_ids,
    torch::Tensor& num_tokens_past_padded, torch::Tensor& topk_weights,
    int64_t moe_block_size, int64_t top_k, bool mul_topk_weights,
    int64_t size_m, int64_t size_n, int64_t size_k, int64_t group_size);
template torch::Tensor marlin_moe_wna16::sm70_marlin_u4_gemm<true>(
    torch::Tensor& a, torch::Tensor& c, torch::Tensor& b_q_weight,
    torch::Tensor& b_scales, torch::Tensor& b_zeros, torch::Tensor& b_bias,
    torch::Tensor& sorted_token_ids, torch::Tensor& expert_ids,
    torch::Tensor& num_tokens_past_padded, torch::Tensor& topk_weights,
