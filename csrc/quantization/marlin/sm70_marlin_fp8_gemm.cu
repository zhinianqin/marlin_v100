...
constexpr char const* kSm70MarlinFp8SplitKEnv = "SM70_MARLIN_FP8_SPLIT_K";
...
class Sm70Fp8IteratorB {
 public:
  ...
  static_assert(ThreadMap::Iterations::kContiguous ==
                ...
                "The SM70 FP8 IteratorB expects one contiguous iteration "
  ...
  int scale_group(int logical_k) const {
    if constexpr (kGroupSize == -1) {
      ...
      static_assert(kGroupSize == 32 || kGroupSize == 64 ||
                        kGroupSize == 128,
                    "SM70 FP8 prototype only specializes group sizes "
                    "-1, 32, 64, and 128.");
...
torch::Tensor launch_sm70_marlin_fp8_gemm(
  ...
  TORCH_CHECK(size_k % int64_t(kCtaK) == 0, kSm70MarlinFp8SplitKEnv,
...
torch::Tensor sm70_marlin_fp8_gemm(torch::Tensor& a, torch::Tensor& c,
  ...
  int const split_k = parse_sm70_dense_split_k(kSm70MarlinFp8SplitKEnv);
