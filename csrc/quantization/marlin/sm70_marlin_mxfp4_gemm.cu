...
torch::Tensor launch_sm70_marlin_mxfp4_gemm(
  ...
  if (requested_split_k == 1) {
    ...
    if constexpr (HasBias) {
      auto kernel = sm70_marlin_mxfp4_gemm_kernel<
...
struct Sm70Mxfp4Launcher {
  ...
  torch::Tensor operator()() const {
    return launch_sm70_marlin_mxfp4_gemm<HasBias, CtaM, CtaN, CtaK, Warps, WarpM, WarpN, WarpK,
...
torch::Tensor sm70_marlin_mxfp4_gemm(torch::Tensor& a, torch::Tensor& c,
  ...
  auto const params = sm70_marlin_dense_auto_params(
      "mxfp4", group_size, HasBias, size_m, size_n, size_k);
  ...
  Sm70Mxfp4Launcher<HasBias> const launcher{
      a, c, b_q_weight, b_scales, b_bias, size_m, size_n, size_k,
...
template torch::Tensor sm70_marlin_mxfp4_gemm<false>(
    torch::Tensor& a, torch::Tensor& c, torch::Tensor& b_q_weight,
    torch::Tensor& b_scales, torch::Tensor& b_bias, int64_t size_m,
    int64_t size_n, int64_t size_k, int64_t group_size);
template torch::Tensor sm70_marlin_mxfp4_gemm<true>(
    torch::Tensor& a, torch::Tensor& c, torch::Tensor& b_q_weight,
    torch::Tensor& b_scales, torch::Tensor& b_bias, int64_t size_m,
    int64_t size_n, int64_t size_k, int64_t group_size);
