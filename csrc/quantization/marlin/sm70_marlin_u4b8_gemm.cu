...
torch::Tensor launch_sm70_marlin_u4b8_gemm(
    ...
    torch::Tensor& b_scales, torch::Tensor& b_bias, int64_t size_m,
  ...
  if (requested_split_k == 1) {
    ...
    if constexpr (HasBias) {
      auto kernel = sm70_marlin_u4b8_gemm_kernel<
...
struct Sm70U4B8Launcher {
  ...
  torch::Tensor operator()() const {
    return launch_sm70_marlin_u4b8_gemm<HasBias, CtaM, CtaN, CtaK, Warps, WarpM, WarpN,
...
torch::Tensor sm70_marlin_u4b8_gemm(torch::Tensor& a, torch::Tensor& c,
  ...
  auto const params = sm70_marlin_dense_auto_params(
      "uint4b8", group_size, HasBias, size_m, size_n, size_k);
...
template torch::Tensor sm70_marlin_u4b8_gemm<false>(
    torch::Tensor& a, torch::Tensor& c, torch::Tensor& b_q_weight,
    torch::Tensor& b_scales, torch::Tensor& b_bias, int64_t size_m,
    int64_t size_n, int64_t size_k, int64_t group_size);
template torch::Tensor sm70_marlin_u4b8_gemm<true>(
    torch::Tensor& a, torch::Tensor& c, torch::Tensor& b_q_weight,
    torch::Tensor& b_scales, torch::Tensor& b_bias, int64_t size_m,
    int64_t size_n, int64_t size_k, int64_t group_size);
