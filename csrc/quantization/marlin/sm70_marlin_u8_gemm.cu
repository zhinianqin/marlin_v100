...
class Sm70U8ZpBiasIteratorB {
 ...
 public:
  ...
  void load_residue_tile(Fragment& frag) const {
    // Current 128x256x32 / 8-warp u8 scale+bias residue instances for
    // GroupSize=32/64/128 compile at 255 REG with a small spill. The u8 path
    // keeps the u4 structure and pays one extra qword per access; residue
    // spill cleanup is tracked separately from the full-tile hot path.
    CUTLASS_PRAGMA_UNROLL
    for (int s = 0; s < ThreadMap::Iterations::kStrided; ++s) {
      int const logical_k =
...
void sm70_marlin_u8_gemm_kernel(
  ...
  using Traits = Sm70U8ZpBiasGemmTraits<GroupSize, FullTile>;
...
torch::Tensor launch_sm70_marlin_u8_gemm(
  ...
  auto kernel = sm70_marlin_u8_gemm_kernel<GroupSize, FullTile>;
  size_t smem_bytes =
      sizeof(typename Sm70U8ZpBiasGemmTraits<GroupSize, FullTile>::SharedStorage);
...
torch::Tensor launch_sm70_marlin_u8_gemm_group_size(
  ...
  switch (group_size) {
    case -1:
      return launch_sm70_marlin_u8_gemm<-1, FullTile>(
          a, c, b_q_weight, b_scales, b_zp_bias, size_m, size_n, size_k);
    case 32:
      return launch_sm70_marlin_u8_gemm<32, FullTile>(
          a, c, b_q_weight, b_scales, b_zp_bias, size_m, size_n, size_k);
    case 64:
      return launch_sm70_marlin_u8_gemm<64, FullTile>(
          a, c, b_q_weight, b_scales, b_zp_bias, size_m, size_n, size_k);
    case 128:
      return launch_sm70_marlin_u8_gemm<128, FullTile>(
          a, c, b_q_weight, b_scales, b_zp_bias, size_m, size_n, size_k);
    default:
      TORCH_CHECK(false,
...
torch::Tensor sm70_marlin_u8_gemm(torch::Tensor& a, torch::Tensor& c,
  ...
  if (full_tile) {
    return launch_sm70_marlin_u8_gemm_group_size<true>(
        a, c, b_q_weight, b_scales, b_zp_bias, size_m, size_n, size_k,
        group_size);
  }
  return launch_sm70_marlin_u8_gemm_group_size<false>(
      a, c, b_q_weight, b_scales, b_zp_bias, size_m, size_n, size_k,
      group_size);
}
