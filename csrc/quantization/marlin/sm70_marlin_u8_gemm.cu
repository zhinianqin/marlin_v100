...
torch::Tensor launch_sm70_marlin_u8_gemm(
  ...
  TORCH_CHECK(size_k % int64_t(CtaK) == 0,
              "SM70 Marlin U8 requires K divisible by CTA_K=",
...
torch::Tensor sm70_marlin_u8_gemm(torch::Tensor& a, torch::Tensor& c,
  ...
  auto const params =
      sm70_marlin_dense_auto_params("U8", group_size, size_m, size_n, size_k);
  ...
  if (params.use_metadata_vector_words) {
    ...
    return dispatch_sm70_marlin_geometry(
        launcher, geometry, params.packed_macro_n, group_size, "U8");
  }
