...
torch::Tensor launch_sm70_marlin_u8b128_gemm(
  ...
  if (requested_split_k == 1) {
    ...
    if constexpr (HasBias) {
      auto kernel = sm70_marlin_u8b128_gemm_kernel<
...
struct Sm70U8B128Launcher {
  ...
  torch::Tensor operator()() const {
    return launch_sm70_marlin_u8b128_gemm<HasBias, CtaM, CtaN, CtaK, Warps, WarpM,
                                          WarpN, WarpK, GroupSize,
                                          PackedMacroN,
                                          UseMetadataVectorWords>(
...
template torch::Tensor sm70_marlin_u8b128_gemm<false>(
    torch::Tensor& a, torch::Tensor& c, torch::Tensor& b_q_weight,
    torch::Tensor& b_scales, torch::Tensor& b_bias, int64_t size_m,
    int64_t size_n, int64_t size_k, int64_t group_size);
template torch::Tensor sm70_marlin_u8b128_gemm<true>(
    torch::Tensor& a, torch::Tensor& c, torch::Tensor& b_q_weight,
    torch::Tensor& b_scales, torch::Tensor& b_bias, int64_t size_m,
    int64_t size_n, int64_t size_k, int64_t group_size);
