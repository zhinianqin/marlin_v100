...
def _reverse_awq_pack_order(size_bits: int) -> list[int]:
    # AWQ uses a non-standard packing order within int32 values. These
    # permutations reverse that ordering for the supported integer widths.
    if size_bits == 4:
        return [0, 4, 1, 5, 2, 6, 3, 7]
    if size_bits == 8:
        return [0, 2, 1, 3]
    raise ValueError(f"Unsupported AWQ Marlin weight bit-width: {size_bits}")
...
def _convert_awq_to_standard_format(
    ...
    reverse_order = torch.tensor(
        _reverse_awq_pack_order(size_bits), dtype=torch.long, device=device
    )
...
class AWQMarlinMoEMethod(FusedMoEMethodBase):
    def __init__(
        ...
        super().__init__(moe)
        self.quant_config = quant_config
        if self.quant_config.weight_bits not in self.quant_config.TYPE_MAP:
            raise ValueError(
                "AWQMarlinMoEMethod only supports 4-bit and 8-bit weights now."
            )
        self.quant_type = self.quant_config.quant_type
        self.input_dtype = None
    ...
    def create_weights(
        ...
        **extra_weight_attrs,
        ...
        strategy = (
            ...
            if self.quant_config.group_size == -1
            else FusedMoeWeightScaleSupported.GROUP.value
        )
        extra_weight_attrs.update({"is_transposed": True, "quant_method": strategy})
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        marlin_w13_zp = moe_awq_to_sm70_marlin_zero_points_float(
            ...
            num_bits=self.quant_config.weight_bits,
            scale_size_k=layer.intermediate_size_per_partition,
            group_size=self.quant_config.group_size,
            is_a_8bit=is_a_8bit,
        )
